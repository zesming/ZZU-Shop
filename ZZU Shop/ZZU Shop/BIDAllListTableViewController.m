...
- (void)getUserAllList
{
    if (!self.userGetAllList.requestError) {
        ...
        if (resultCode == 200){
            self.allListData = [NSMutableArray new];
            NSArray *a = resultDic[@"orderList"];
            [self.allListData addObjectsFromArray:a];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.productNumLabel.text = [NSString stringWithFormat:@"x%@", self.allListData[indexPath.row][@"order"][@"pnum"]];
